...
```ts
  ...
  20 |         }
  21 |         const tempo = await page.locator('#msg').innerHTML({timeout:600});
...
> 23 |         await expect(tempo).toBe('Login successful');
  ...
  69 |     const tempo = await page.locator('#msg').textContent({timeout:600});
  70 |     console.log('Menssage recebida: \n', tempo)
  71 |     await expect(page.locator('#msg')).toHaveText('Login successful');
